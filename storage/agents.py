...
async def create_agent(name: str, instructions: str, tools: list[str], model: str | None, output_type: str | None) -> str:
    agent_id = str(uuid.uuid4())
    now = time.time()
    async with aiosqlite.connect(DB_PATH) as db:
        await _ensure_schema(db)
        await db.execute(
            "INSERT INTO saved_agents (id, name, instructions, tools_json, model, output_type, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (agent_id, name, instructions, json.dumps(tools), model, output_type, now, now),
        )
        await db.commit()
    return agent_id
...
async def list_agents() -> list[dict]:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(
            "SELECT id, name, instructions, tools_json, model, output_type, created_at, updated_at "
